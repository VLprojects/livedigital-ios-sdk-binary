...
При запуске приложения показывается экран `FlowSelectionView` с выбором сценария (звонки или конференц-комнаты). Выбор сохраняется и для его смены нужно удалить и установить приложение заново.

### Сценарий со звонками

Экран `CallStartScreen` содержит кнопки-индикаторы для управления разрешениями и поле ввода своего номера телефона.

Основная работа с SDK сосредоточена в классе `AudioCallVM`, который отвечает за сессию "звонка" или "конференции". Для сохранения простоты Example проекта, он не реализует полноценной обработки ситуации когда разрешения не выданы. После выдачи разрешений происходит подключение к комнате. В верхней части экрана отображается локальный участник с кнопками включения микрофона и камеры. Основную часть экрана занимает сетка участников с горизонтальной прокруткой.
